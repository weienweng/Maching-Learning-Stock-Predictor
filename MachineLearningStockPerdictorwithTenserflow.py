...
class RotatingIndicator(QWidget):
    def __init__(self, parent=None):
        ...
        self.setFixedSize(24, 24)  # Reduced size
    ...
    def paintEvent(self, event):
        ...
        if self.state == 'idle':
            ...
            for i in range(8):
                ...
                painter.drawEllipse(9, -3, 6, 6)  # Adjusted size
                painter.rotate(45)
        elif self.state == 'completed':
            painter.setPen(Qt.green)
            painter.setFont(QFont('Arial', 15))  # Adjusted font size
            painter.drawText(self.rect(), Qt.AlignCenter, "✓")
...
class LSTMModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, output_dim, dropout=0.2):
        ...
        # TensorFlow LSTM
        self.tf_lstm = tf.keras.Sequential([
        ...
        self.tf_lstm.compile(optimizer='adam', loss='mse')
    ...
    def fit_ensemble(self, X, y):
        # Fit TensorFlow LSTM
        self.tf_lstm.fit(X.cpu().numpy(), y.cpu().numpy().ravel(), epochs=100, verbose=0)
        
        # Fit Random Forest
        X_2d = X.reshape(X.shape[0], -1).cpu().numpy()
        self.rf_model.fit(X_2d, y.cpu().numpy().ravel())
    
    def predict_ensemble(self, X):
        # PyTorch LSTM prediction
        self.eval()
        with torch.no_grad():
            pytorch_pred = self(X).cpu().numpy()
        
        # TensorFlow LSTM prediction
        tf_pred = self.tf_lstm.predict(X.cpu().numpy())
        
        # Random Forest prediction
        X_2d = X.reshape(X.shape[0], -1).cpu().numpy()
        rf_pred = self.rf_model.predict(X_2d)
        
        # Ensemble prediction (simple average)
        ensemble_pred = (pytorch_pred + tf_pred + rf_pred.reshape(-1, 1)) / 3
...
class StockPredictionThread(QThread):
    ...
    def predict_future(self, model, scaled_data, scaler, days=7):
        ...
        for _ in range(days):
            with torch.no_grad():
                prediction = model.predict_ensemble(last_sequence)[0, 0]
    ...
    def run_lstm_model(self, processed_data, scaler, original_close):
        ...
        for fold, (train_index, val_index) in enumerate(tscv.split(X), 1):
            ...
            self.update_signal.emit(f"Calculating [spinning] please wait (Fold {fold}/5)")
            ...
            model.eval()
            predictions = model.predict_ensemble(X_val)
        ...
        self.window.signal_emitter.start_spinner.emit()
        self.update_signal.emit("Calculating [spinning] please wait (Final Model)")
        ...
        predictions = final_model.predict_ensemble(X)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Chart tabs
        self.chart_tabs = {}
        for model in ['Ensemble', 'LSTM', 'ARIMA', 'Random Forest', 'Technical Analysis']:
            tab = QWidget()
    ...
    def search_stock(self):
        ...
        self.signal_emitter.update_indicator_state.emit('running')  # Start spinning indicator
        ...
        if not history.empty:
            ...
            self.signal_emitter.update_indicator_state.emit('idle')  # Stop spinning indicator
            return
    ...
    def toggle_pause(self):
        ...
        # Enable other buttons when paused
        self.start_button.setEnabled(self.prediction_thread.is_paused)
        self.recalculate_button.setEnabled(self.prediction_thread.is_paused)
        self.import_button.setEnabled(self.prediction_thread.is_paused)
        self.search_bar.setEnabled(self.prediction_thread.is_paused)
    ...
    def update_chart(self, results):
        try:
            ...
            self.update_single_chart('Ensemble', results)
    ...
    def show_results(self, results):
        ...
        self.pause_button.setEnabled(False)
        self.recalculate_button.setEnabled(True)
        self.rotating_indicator.set_state('completed')  # Show checkmark when prediction is completed
